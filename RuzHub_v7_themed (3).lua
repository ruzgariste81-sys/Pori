-- ================================================================
--  RuzHub Mmv And Mm2  v7.3
--  * Duel mode support (knife vs knife / gun vs gun)
--  * Skybox picker: selecting a preset applies immediately, no toggle needed
--  * Crosshair picker: Spin toggle inside picker
--  * All text in English
-- ================================================================
...
local function OpenSliderPopup(title, minVal, maxVal, defaultVal, step, onApply, onReset)
    ...
    frame.BackgroundTransparency = 0.08; frame.BorderSizePixel = 0
    Instance.new("UICorner", frame).CornerRadius = UDim.new(0, 10)
    local fs = Instance.new("UIStroke", frame)
    fs.Color = Color3.fromRGB(220, 38, 38); fs.Thickness = 1.5; fs.Transparency = 0.15

    local hdr = Instance.new("TextLabel", frame)
    hdr.Size = UDim2.new(1, -44, 0, 36); hdr.Position = UDim2.new(0, 12, 0, 0)
    hdr.BackgroundTransparency = 1; hdr.Text = "RuzHub  —  " .. title
    hdr.TextColor3 = Color3.fromRGB(255, 255, 255); hdr.Font = Enum.Font.GothamBold
    hdr.TextSize = 14; hdr.TextXAlignment = Enum.TextXAlignment.Left

    local xBtn = Instance.new("TextButton", frame)
    xBtn.Size = UDim2.new(0, 28, 0, 28); xBtn.Position = UDim2.new(1, -34, 0, 4)
    xBtn.BackgroundColor3 = Color3.fromRGB(180, 30, 30); xBtn.Text = "X"
    xBtn.TextColor3 = Color3.new(1, 1, 1); xBtn.Font = Enum.Font.GothamBold; xBtn.TextSize = 13
    Instance.new("UICorner", xBtn).CornerRadius = UDim.new(0, 6)
    xBtn.MouseButton1Click:Connect(function() sg:Destroy() end)

    local currentVal = defaultVal
    local valLbl = Instance.new("TextLabel", frame)
    valLbl.Size = UDim2.new(1, 0, 0, 22); valLbl.Position = UDim2.new(0, 0, 0, 38)
    valLbl.BackgroundTransparency = 1; valLbl.Text = title .. ":  " .. tostring(currentVal)
    valLbl.TextColor3 = Color3.fromRGB(210, 210, 210); valLbl.Font = Enum.Font.Gotham; valLbl.TextSize = 13

    local track = Instance.new("Frame", frame)
    track.Size = UDim2.new(1, -30, 0, 10); track.Position = UDim2.new(0, 15, 0, 72)
    track.BackgroundColor3 = Color3.fromRGB(45, 45, 45); track.BorderSizePixel = 0
    Instance.new("UICorner", track).CornerRadius = UDim.new(1, 0)

    local r0   = (currentVal - minVal) / (maxVal - minVal)
    local fill = Instance.new("Frame", track)
    fill.Size = UDim2.new(r0, 0, 1, 0)
    fill.BackgroundColor3 = Color3.fromRGB(220, 38, 38); fill.BorderSizePixel = 0
    ...
    handle.AutoButtonColor = false; handle.BorderSizePixel = 0
    ...
    UserInputService.InputChanged:Connect(function(i)
        ...
        if i.UserInputType == Enum.UserInputType.MouseMovement or i.UserInputType == Enum.UserInputType.Touch then updateFromX(i.Position.X) end
    end)
    UserInputService.InputEnded:Connect(function(i)
    ...
    btnRow.Size = UDim2.new(1, -20, 0, 36); btnRow.Position = UDim2.new(0, 10, 0, 126)
    ...
    applyBtn.Text = "Apply"; applyBtn.TextColor3 = Color3.new(1, 1, 1)
...
predPart.Name = "RuzPredictionPart"; predPart.Size = Vector3.new(0.5, 0.5, 0.5)
predPart.Anchored = true; predPart.CanCollide = false
predPart.Transparency = 1; predPart.Parent = Workspace
...
local function PlaceGunMarker(pos)
    ...
    p.Name = "RuzGunMarker"; p.Size = Vector3.new(1.5, 0.15, 1.5)
    p.Anchored = true; p.CanCollide = false; p.CastShadow = false
    p.Material = Enum.Material.Neon; p.Color = Color3.fromRGB(50, 255, 80)
    p.Transparency = 0.25; p.CFrame = CFrame.new(pos); p.Parent = Workspace
    task.spawn(function()
...
local function ApplyGunESP(gunDrop)
    ...
    hl.Adornee = gunDrop; hl.FillColor = gunHLColor
    ...
    if handle then
        ...
        bb.Adornee = handle; bb.Size = UDim2.new(0, 130, 0, 36)
        bb.StudsOffset = Vector3.new(0, 4, 0); bb.AlwaysOnTop = true
        bb.MaxDistance = 300; bb.Parent = handle
        ...
        bgS.Color = gunHLColor; bgS.Thickness = 1.5; bgS.Transparency = 0.1
        local lbl = Instance.new("TextLabel", bg)
        lbl.Size = UDim2.new(1, 0, 1, 0); lbl.BackgroundTransparency = 1
        lbl.Text = "GUN ON MAP"; lbl.TextColor3 = gunHLColor
...
-- ROLE ESP
-- ==========================================
local espEnabled  = false; local espConn = nil
local rolesData   = {}; local lastEspTick = 0
...
-- ==========================================
-- TARGET FINDER — with duel support
-- Duel mode: player may also have knife or gun
-- Priority: enemy with knife (murderer/duel knife) > enemy with gun
-- In duel, both sides can have the same weapon type
-- ==========================================
local currentTarget = nil

local function HasKnife(p)
    return p.Backpack:FindFirstChild("Knife") or
           (p.Character and p.Character:FindFirstChild("Knife"))
end
local function HasGun(p)
    return p.Backpack:FindFirstChild("Gun") or
           (p.Character and p.Character:FindFirstChild("Gun"))
end
...
local function FindBestTarget()
    ...
    local myHasKnife = HasKnife(player)
    local myHasGun   = HasGun(player)
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= player and p.Character then
            ...
            local theyHaveKnife = HasKnife(p)
            local theyHaveGun   = HasGun(p)
            local dist = (hrp.Position - myHRP.Position).Magnitude

            local valid = false

            -- Normal MM2 mode: target knife holders first, then gun holders
            -- Duel mode (I also have a weapon): target whoever has same weapon type, or any weapon
            if myHasKnife then
                -- I have knife: target other knife holders (duel) or anyone dangerous
                if theyHaveKnife then valid = true end
            elseif myHasGun then
                -- I have gun: target other gun holders (duel) or knife holders
                if theyHaveGun or theyHaveKnife then valid = true end
            else
                -- I have no weapon: target knife holders first, then gun holders
                if theyHaveKnife then valid = true; dist = dist - 1000 end -- priority boost
    ...
    -- Fallback: no weapon-based target found, pick nearest alive enemy
    if not best then
        for _, p in ipairs(Players:GetPlayers()) do
            if p ~= player and p.Character then
                local c   = p.Character
...
RunService.RenderStepped:Connect(function()
    ...
    local torso = tgt:FindFirstChild("UpperTorso")
               or tgt:FindFirstChild("Torso")
               or tgt:FindFirstChild("HumanoidRootPart")
...
local function AutoKill()
    ...
    local origin    = myHRP.Position + Vector3.new(0, 1.0, 0)
    local shootCF   = CFrame.new(origin, tPos)

    pcall(function()
        gun:WaitForChild("Shoot"):FireServer(shootCF, CFrame.new(tPos))
    end)
end

-- ==========================================
-- KNIFE THROW — torso aimed, duel aware
-- ==========================================
local function ThrowKnife()
    ...
    -- Use FindBestTarget so duel opponents are included
    local tgtChar = currentTarget
    if not tgtChar then
        -- last resort: nearest alive
        ...
        for _, p in ipairs(Players:GetPlayers()) do
            if p ~= player and p.Character then
                ...
                if hrp and hum and hum.Health > 0 then
                    ...
                    if d < nearDist then nearDist = d; tgtChar = p.Character end
                end
            end
        end
    end
    ...
    local dist  = (torso.Position - myHRP.Position).Magnitude
    local extra = 0
    if autoPingPred then
        local ok, ping = pcall(function() return player:GetNetworkPing() end)
        extra = ok and ping or 0
    end
...
-- ==========================================
-- SMART SHOOT — auto-detects weapon
-- ==========================================
...
local function DoSmartFlick()
    ...
    if not isShiftLock then
        ...
        local tgt = CFrame.lookAt(camCF.Position, camCF.Position + newLook)
...
local function DoWallHop()
    ...
    local origCamCF = Camera.CFrame; local STEPS = 7
...
local function ExecuteJump(bombName, isGold)
    ...
    pcall(function()
        bomb.Remote:FireServer(CFrame.new(hrp.Position + hrp.CFrame.LookVector * 1.5 + Vector3.new(0, -3, 0)), 50)
    end)
...
-- SPEED GLITCH
-- ==========================================
local speedEnabled = false; local speedConn = nil
...
-- STRETCH
-- ==========================================
local stretchEnabled = false; local stretchConn = nil
local stretchValue   = 0.50  -- 0.1 = very wide, 1.0 = normal
...
local function OpenStretchSlider()
    -- stretchValue is stored as 0.1–1.0 internally
    -- We show it as 10–100 to the user (multiply by 100)
    local displayVal = math.round(stretchValue * 100)
    OpenSliderPopup(
        "Stretch Resolution",
        10, 100, displayVal, 5,
        function(v)
            stretchValue = v / 100
            -- If stretch is already on, restart connection with new value
            if stretchEnabled then SetStretch(true) end
            Notify("Stretch set to " .. v .. "%  (1.0 = normal)")
        end,
        function()
            stretchValue = 0.50
            if stretchEnabled then SetStretch(true) end
            Notify("Stretch reset to 50%")
        end
    )
end
...
local function DoGrabGun()
    ...
    if gd:IsA("BasePart") then targetPos = gd.Position
        ...
        local part = gd:FindFirstChild("Handle") or gd:FindFirstChildWhichIsA("BasePart") or gd.PrimaryPart
        targetPos = part and part.Position or gd:GetModelCFrame().Position
    end
    if not targetPos then Notify("Gun position not found!"); return end
    local oldCF = hrp.CFrame
    hrp.CFrame = CFrame.new(targetPos + Vector3.new(0, 2, 0))
    task.wait(0.2); hrp.CFrame = oldCF
    Notify("Teleported to gun!")
end

-- ==========================================
-- SKYBOX — selecting a preset applies immediately
-- ==========================================
...
local function RestoreDefaultSky()
    ...
    skyboxActive = false
    Notify("Skybox restored to default.")
end
...
local function ApplySkyboxById(id)
    ...
    sky.SunAngularSize = 0; sky.StarCount = 0
...
local function OpenSkyboxPicker()
    local uid = "RuzSkyboxPicker"
    local ex  = game.CoreGui:FindFirstChild(uid)
    if ex then ex:Destroy(); return end

    local sg = Instance.new("ScreenGui", game.CoreGui)
    sg.Name = uid; sg.ResetOnSpawn = false; sg.DisplayOrder = 62

    local frame = Instance.new("Frame", sg)
    frame.Size = UDim2.new(0, 310, 0, 420)
    frame.Position = UDim2.new(0.5, -155, 0.04, 0)
    frame.BackgroundColor3 = Color3.fromRGB(10, 10, 10)
    frame.BackgroundTransparency = 0.06; frame.BorderSizePixel = 0
    Instance.new("UICorner", frame).CornerRadius = UDim.new(0, 12)
    local fs = Instance.new("UIStroke", frame); fs.Color = Color3.fromRGB(220, 38, 38); fs.Thickness = 1.5

    -- Header
    local hdr = Instance.new("TextLabel", frame)
    hdr.Size = UDim2.new(1, -44, 0, 38); hdr.Position = UDim2.new(0, 12, 0, 0)
    hdr.BackgroundTransparency = 1; hdr.Text = "RuzHub  —  Skybox Picker"
    hdr.TextColor3 = Color3.fromRGB(255, 255, 255); hdr.Font = Enum.Font.GothamBold
    hdr.TextSize = 14; hdr.TextXAlignment = Enum.TextXAlignment.Left

    local xBtn = Instance.new("TextButton", frame)
    xBtn.Size = UDim2.new(0, 28, 0, 28); xBtn.Position = UDim2.new(1, -34, 0, 5)
    xBtn.BackgroundColor3 = Color3.fromRGB(180, 30, 30); xBtn.Text = "X"
    xBtn.TextColor3 = Color3.new(1, 1, 1); xBtn.Font = Enum.Font.GothamBold; xBtn.TextSize = 13
    Instance.new("UICorner", xBtn).CornerRadius = UDim.new(0, 6)
    xBtn.MouseButton1Click:Connect(function() sg:Destroy() end)
    ...
    idBox.Size = UDim2.new(1, -20, 0, 34); idBox.Position = UDim2.new(0, 10, 0, 44)
    idBox.BackgroundColor3 = Color3.fromRGB(25, 25, 25); idBox.Text = ""
    idBox.PlaceholderText = "Enter custom Skybox ID, press Enter..."
    ...
    Instance.new("UIStroke", idBox).Color = Color3.fromRGB(80, 80, 80)
    idBox.FocusLost:Connect(function(enter)
        if enter and idBox.Text ~= "" then
            ApplySkyboxById(idBox.Text)
            Notify("Custom skybox applied — ID: " .. idBox.Text)
            idBox.Text = ""
        end
    end)
    ...
    restoreBtn.Size = UDim2.new(1, -20, 0, 28); restoreBtn.Position = UDim2.new(0, 10, 0, 84)
    restoreBtn.BackgroundColor3 = Color3.fromRGB(40, 40, 40); restoreBtn.Text = "Restore Default Sky"
    restoreBtn.TextColor3 = Color3.fromRGB(200, 200, 200); restoreBtn.Font = Enum.Font.GothamBold
    ...
    sep.Size = UDim2.new(1, -20, 0, 1); sep.Position = UDim2.new(0, 10, 0, 118)
    sep.BackgroundColor3 = Color3.fromRGB(60, 60, 60); sep.BorderSizePixel = 0

    -- Scroll list — click = instant apply
    local scroll = Instance.new("ScrollingFrame", frame)
    scroll.Size = UDim2.new(1, -14, 1, -126); scroll.Position = UDim2.new(0, 7, 0, 124)
    ...
    for i, preset in ipairs(SKYBOX_PRESETS) do
        ...
        row.Size = UDim2.new(1, -8, 0, 48); row.BackgroundColor3 = Color3.fromRGB(20, 20, 20)
        ...
        local colorBox = Instance.new("Frame", row)
        colorBox.Size = UDim2.new(0, 34, 0, 34); colorBox.Position = UDim2.new(0, 8, 0.5, -17)
        colorBox.BackgroundColor3 = preset.color; colorBox.BorderSizePixel = 0
        Instance.new("UICorner", colorBox).CornerRadius = UDim.new(0, 6)

        local nameLbl = Instance.new("TextLabel", row)
        nameLbl.Size = UDim2.new(1, -58, 0, 22); nameLbl.Position = UDim2.new(0, 50, 0, 6)
        nameLbl.BackgroundTransparency = 1; nameLbl.Text = preset.name
        nameLbl.TextColor3 = Color3.fromRGB(210, 210, 210); nameLbl.Font = Enum.Font.GothamBold
        nameLbl.TextSize = 14; nameLbl.TextXAlignment = Enum.TextXAlignment.Left

        local idLbl = Instance.new("TextLabel", row)
        idLbl.Size = UDim2.new(1, -58, 0, 14); idLbl.Position = UDim2.new(0, 50, 1, -18)
        idLbl.BackgroundTransparency = 1; idLbl.Text = "ID: " .. preset.id
        idLbl.TextColor3 = Color3.fromRGB(100, 100, 100); idLbl.Font = Enum.Font.Gotham
        idLbl.TextSize = 10; idLbl.TextXAlignment = Enum.TextXAlignment.Left

        row.MouseButton1Click:Connect(function()
            -- Instantly apply, no toggle needed
            ApplySkyboxById(preset.id)
            Notify("Skybox applied: " .. preset.name)
            -- Visual feedback: highlight selected
            for _, child in ipairs(scroll:GetChildren()) do
                if child:IsA("TextButton") then
                    local st = child:FindFirstChildOfClass("UIStroke")
                    if st then st.Thickness = 1; st.Color = Color3.fromRGB(80, 80, 80) end
                    child.BackgroundColor3 = Color3.fromRGB(20, 20, 20)
                end
            end
            rs.Thickness = 2; rs.Color = Color3.fromRGB(220, 38, 38)
            row.BackgroundColor3 = Color3.fromRGB(50, 15, 15)
...
-- ANTI-FLING
-- ==========================================
local antiFling = false; local antiFlingConn = nil
...
local function SkidFling(targetPlayer)
    ...
    bv.Velocity = Vector3.new(0, 0, 0); bv.MaxForce = Vector3.new(9e9, 9e9, 9e9); bv.Parent = myHRP
...
local function EnableLowGraphics()
    ...
    lgStarLbl.Visible = true; Notify("Low Graphics ON — FPS boost active")
end
...
local function SetupCrosshairDisplay()
    ...
    RunService.RenderStepped:Connect(function()
        ...
        if pg then
            ...
            if tb and tb:FindFirstChild("Crosshair") then tb.Crosshair.Visible = false end
        end
...
-- ==========================================
-- CROSSHAIR PICKER UI — with spin toggle inside
-- ==========================================
local function OpenCursorPicker()
    ...
    local sg = Instance.new("ScreenGui", game.CoreGui)
    sg.Name = uid; sg.ResetOnSpawn = false; sg.DisplayOrder = 60
    ...
    local hdr = Instance.new("TextLabel", frame)
    hdr.Size = UDim2.new(1, -44, 0, 38); hdr.Position = UDim2.new(0, 12, 0, 0)
    hdr.BackgroundTransparency = 1; hdr.Text = "RuzHub  —  Cursor Picker"
    hdr.TextColor3 = Color3.fromRGB(255, 255, 255); hdr.Font = Enum.Font.GothamBold
    hdr.TextSize = 14; hdr.TextXAlignment = Enum.TextXAlignment.Left
    ...
    idBox.Size = UDim2.new(1, -20, 0, 34); idBox.Position = UDim2.new(0, 10, 0, 44)
    idBox.BackgroundColor3 = Color3.fromRGB(25, 25, 25); idBox.Text = ""
    idBox.PlaceholderText = "Enter custom Cursor ID, press Enter..."
    idBox.TextColor3 = Color3.new(1, 1, 1); idBox.PlaceholderColor3 = Color3.fromRGB(120, 120, 120)
    idBox.Font = Enum.Font.Gotham; idBox.TextSize = 13; idBox.ClearTextOnFocus = false
    Instance.new("UICorner", idBox).CornerRadius = UDim.new(0, 6)
    Instance.new("UIStroke", idBox).Color = Color3.fromRGB(80, 80, 80)
    idBox.FocusLost:Connect(function(enter)
        if enter and idBox.Text ~= "" then
            ...
            Notify("Custom cursor applied — enable ShiftLock to see it!"); idBox.Text = ""
        end
    end)

    -- Spin toggle row
    local spinRow = Instance.new("Frame", frame)
    spinRow.Size = UDim2.new(1, -20, 0, 30); spinRow.Position = UDim2.new(0, 10, 0, 84)
    spinRow.BackgroundTransparency = 1

    local spinLbl = Instance.new("TextLabel", spinRow)
    spinLbl.Size = UDim2.new(1, -64, 1, 0); spinLbl.BackgroundTransparency = 1
    spinLbl.Text = "Spin Crosshair"
    spinLbl.TextColor3 = Color3.fromRGB(200, 200, 200); spinLbl.Font = Enum.Font.GothamBold
    spinLbl.TextSize = 13; spinLbl.TextXAlignment = Enum.TextXAlignment.Left

    local spinBtn = Instance.new("TextButton", spinRow)
    spinBtn.Size = UDim2.new(0, 54, 0, 26); spinBtn.Position = UDim2.new(1, -54, 0.5, -13)
    ...
    spinBtn.TextColor3 = Color3.new(1, 1, 1); spinBtn.Font = Enum.Font.GothamBold; spinBtn.TextSize = 12
    ...
    spinBtn.MouseButton1Click:Connect(function()
        ...
        spinBtn.Text = crosshairSpin and "ON" or "OFF"
        UpdateCrosshairSpin()
        Notify("Crosshair Spin: " .. (crosshairSpin and "ON" or "OFF"))
    end)

    -- Separator
    local sep = Instance.new("Frame", frame)
    sep.Size = UDim2.new(1, -20, 0, 1); sep.Position = UDim2.new(0, 10, 0, 120)
    sep.BackgroundColor3 = Color3.fromRGB(60, 60, 60); sep.BorderSizePixel = 0

    -- Grid
    local scroll = Instance.new("ScrollingFrame", frame)
    scroll.Size = UDim2.new(1, -14, 1, -128); scroll.Position = UDim2.new(0, 7, 0, 126)
    scroll.BackgroundTransparency = 1; scroll.BorderSizePixel = 0; scroll.ScrollBarThickness = 4
    scroll.CanvasSize = UDim2.new(0, 0, 0, math.ceil(#CURSORS / 2) * 118 + 10)
    local grid = Instance.new("UIGridLayout", scroll)
    grid.CellSize = UDim2.new(0, 128, 0, 110); grid.CellPadding = UDim2.new(0, 8, 0, 8)
    grid.SortOrder = Enum.SortOrder.LayoutOrder

    for i, cursor in ipairs(CURSORS) do
        local isActive = (activeCursorId == cursor.id)
        local cell = Instance.new("TextButton", scroll)
        cell.Size = UDim2.new(0, 128, 0, 110)
        ...
        cs.Thickness = isActive and 1.8 or 1.2

        local img = Instance.new("ImageLabel", cell)
        img.Size = UDim2.new(0, 58, 0, 58); img.AnchorPoint = Vector2.new(0.5, 0)
        img.Position = UDim2.new(0.5, 0, 0, 8); img.BackgroundTransparency = 1
        img.Image = "rbxassetid://" .. cursor.id

        local nameLbl = Instance.new("TextLabel", cell)
        nameLbl.Size = UDim2.new(1, -6, 0, 28); nameLbl.Position = UDim2.new(0, 3, 1, -30)
        nameLbl.BackgroundTransparency = 1
        nameLbl.Text = cursor.name .. (isActive and " ✓" or "")
        ...
        cell.MouseButton1Click:Connect(function()
            ...
            Notify("Cursor: " .. cursor.name .. " — enable ShiftLock to see it!")
            sg:Destroy()
        end)
    end
...
local btnGui = (function()
    ...
    local sg = Instance.new("ScreenGui", game.CoreGui)
    sg.Name = "RuzHub_BtnLayer"; sg.ResetOnSpawn = false
...
-- Tüm butonların gradient rotasyonunu tek bir loop'ta yönet
local btnAnimList = {} -- { strokeGrad, imgRef }
RunService.RenderStepped:Connect(function()
    local r = (tick() * 90) % 360 -- saniyede 90 derece = smooth
    for _, entry in ipairs(btnAnimList) do
        if entry.strokeGrad and entry.strokeGrad.Parent then
            entry.strokeGrad.Rotation = r
        end
        if entry.img and entry.img.Parent then
            entry.img.Rotation = -r
        end
    end
end)

local RED_A = Color3.fromRGB(255, 0,  0)
local RED_C = Color3.fromRGB(20,  0,  0)
...
local function AddDragBtn(btn)
    ...
    btn.InputChanged:Connect(function(i)
        ...
        if i.UserInputType == Enum.UserInputType.MouseMovement or i.UserInputType == Enum.UserInputType.Touch then
            ...
            TweenService:Create(btn, TweenInfo.new(0.08, Enum.EasingStyle.Linear), {
...
local function NewBtn(key, pos, size, label)
    ...
    -- Basma animasyonu (v5 ile aynı)
    btn.MouseButton1Down:Connect(function()
        TweenService:Create(btn, TweenInfo.new(0.1), {Size = UDim2.new(0, size.X.Offset - 7, 0, size.Y.Offset - 7)}):Play()
    end)
    btn.MouseButton1Up:Connect(function()
        TweenService:Create(btn, TweenInfo.new(0.1), {Size = size}):Play()
    end)

    -- Gradient outline (v5 ile birebir: kırmızı → siyah → kırmızı, dönen)
    ...
    strokeGrad.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0,   RED_A),
        ColorSequenceKeypoint.new(0.5, RED_C),
        ColorSequenceKeypoint.new(1,   RED_A),
    })

    -- Label - gradient yazı (kırmızı → koyu kırmızı)
    ...
    lbl.TextColor3 = Color3.fromRGB(255, 255, 255) -- UIGradient override eder
    ...
    AddDragBtn(btn)
    local entry = { btn = btn, stroke = stroke, strokeGrad = strokeGrad, lbl = lbl, lblGrad = lblGrad, img = nil }
    btnRefs[key] = entry
    table.insert(btnAnimList, entry)
    return entry
end
...
local function AddSpinImg(ref, texId)
    ...
    img.Position = UDim2.new(0.5, 0, 0.40, 0) -- ortada biraz yukarı, yazı alta kalıyor
    img.BackgroundTransparency = 1
    img.Image = "rbxassetid://"..tostring(texId)
    img.ScaleType = Enum.ScaleType.Fit
    img.ImageColor3 = Color3.fromRGB(255, 50, 50) -- v5 ile aynı kırmızı
    ref.img = img
    -- Yazı aşağıya taşı
    ref.lbl.Size     = UDim2.new(1, 0, 0.28, 0)
    ref.lbl.Position = UDim2.new(0, 0, 0.72, 0)
    ref.lbl.TextSize = math.max(9, ref.btn.Size.Y.Offset * 0.12)
end
...
local function LoadGoldBomb(v)
    ...
    NewBtn("GoldBomb", DEFAULT_POS.GoldBomb, BIG, "GOLD\nJUMP")
...
local function LoadNormalBomb(v)
    ...
    NewBtn("NormalBomb", DEFAULT_POS.NormalBomb, BIG, "NORMAL\nJUMP")
...
local function LoadShoot(v)
    ...
    local ref = NewBtn("Shoot", DEFAULT_POS.Shoot, BIG, "SHOOT")
...
local function LoadESP(v)
    ...
    NewBtn("ESP", DEFAULT_POS.ESP, SMALL, "ESP\nOFF")
...
local function LoadFlick(v)
    ...
    NewBtn("Flick", DEFAULT_POS.Flick, SMALL, "FLICK")
...
local function LoadSpeed(v)
    ...
    NewBtn("Speed", DEFAULT_POS.Speed, SMALL, "SPEED")
    btnRefs.Speed.btn.MouseButton1Click:Connect(function()
        speedEnabled = not speedEnabled; Notify(speedEnabled and "Speed Glitch ON" or "Speed Glitch OFF")
    end)
end
local function LoadStretch(v)
    ...
    NewBtn("Stretch", DEFAULT_POS.Stretch, SMALL, "STRETCH")
    btnRefs.Stretch.btn.MouseButton1Click:Connect(function()
        stretchEnabled = not stretchEnabled; SetStretch(stretchEnabled)
...
local function LoadGrabGun(v)
    ...
    NewBtn("GrabGun", DEFAULT_POS.GrabGun, SMALL, "GRAB\nGUN")
...
local function LoadWallHop(v)
    ...
    NewBtn("WallHop", DEFAULT_POS.WallHop, SMALL, "WALL\nHOP")
...
local function LoadFlingMurderer(v)
    ...
    NewBtn("FlingMurderer", DEFAULT_POS.FlingMurderer, SMALL, "FLING\nMURD")
...
local function LoadFlingSheriff(v)
    ...
    NewBtn("FlingSheriff", DEFAULT_POS.FlingSheriff, SMALL, "FLING\nSHERIF")
...
RunService.Heartbeat:Connect(function()
    ...
    if btnRefs.FlingMurderer then
        local hasMurd = false
        for _, p in ipairs(Players:GetPlayers()) do
            if p ~= player and HasKnife(p) then hasMurd = true; break end
        end
    ...
    if btnRefs.FlingSheriff then
        ...
        for _, p in ipairs(Players:GetPlayers()) do
            if p ~= player and HasGun(p) then hasSheriff = true; break end
        end
...
--  MAIN TAB
-- ================================================================
MainTab:Paragraph({ Title = "Auto-Loaded Buttons", Content = "Gold Bomb, Normal Bomb and Shoot/Throw are enabled by default." })
...
MainTab:Paragraph({ Title = "Optional Buttons", Content = "Toggle to add or remove from screen." })
...
MainTab:Button({ Title = "Stretch Resolution Slider", Description = "10% = very wide  /  100% = normal", Callback = OpenStretchSlider })
...
MainTab:Toggle({ Title = "Load Wall Hop",        Default = false, Callback = function(v) LoadWallHop(v)       end })
MainTab:Divider()

MainTab:Paragraph({ Title = "Skybox", Content = "Click the button below to open the visual skybox picker.\nSelecting a preset applies it instantly." })
MainTab:Button({ Title = "Open Skybox Picker", Description = "Color preview list — click to apply instantly", Callback = OpenSkyboxPicker })
MainTab:Button({ Title = "Restore Default Sky", Callback = function() RestoreDefaultSky() end })
MainTab:Divider()

MainTab:Paragraph({ Title = "Crosshair", Content = "Visible only when ShiftLock is active.\nSpin option is inside the picker." })
MainTab:Toggle({
    Title = "Enable Custom Crosshair", Description = "Visible only while ShiftLock is on", Default = false,
    Callback = function(v)
        crosshairActive = v
        if v then
            SetupCrosshairDisplay()
            Notify("Crosshair ON — enable ShiftLock to see it!")
        else
...
MainTab:Button({ Title = "Open Cursor Picker", Description = "Visual grid with spin toggle — click to apply", Callback = OpenCursorPicker })
MainTab:Divider()

MainTab:Paragraph({ Title = "Graphics", Content = "Low: removes textures, map looks flat, boosts FPS.\nHigh: Bloom, SunRays, enhanced lighting." })
MainTab:Toggle({ Title = "Low Graphics (FPS Boost)",  Default = false, Callback = function(v) if v then EnableLowGraphics()  else DisableLowGraphics()  end end })
MainTab:Toggle({ Title = "High Graphics (Beautiful)", Default = false, Callback = function(v) if v then EnableHighGraphics() else DisableHighGraphics() end end })
MainTab:Button({
    Title = "FOV Slider", Description = "Mobile-friendly field of view selector",
    Callback = function()
...
MainTab:Paragraph({ Title = "Extra Scripts", Content = "Universal scripts and additional tools." })
MainTab:Button({
    Title = "Load Emotes GUI", Description = "7yd7 emote panel",
    Callback = function()
...
MainTab:Button({
    Title = "Load Infinite Yield", Description = "Admin script",
    Callback = function()
        local ok, err = pcall(function()
...
MainTab:Toggle({ Title = "Anti-Fling", Description = "Limits velocity to prevent being launched", Default = false,
    Callback = function(v) SetAntiFling(v); Notify(v and "Anti-Fling ON" or "Anti-Fling OFF") end })
MainTab:Toggle({ Title = "Auto Ping Prediction", Description = "Adds ping offset to shoot and throw", Default = false,
    Callback = function(v) autoPingPred = v; Notify(v and "Ping Prediction ON" or "Ping Prediction OFF") end })
MainTab:Button({
    Title = "Speed Glitch Slider", Description = "Mobile-friendly speed selector",
    Callback = function()
...
MainTab:Dropdown({ Title = "Velocity Cap (Anti-Fling)", Options = { "50","100","150","200","300","500" }, Default = "200",
    Callback = function(v) MAX_VELOCITY = tonumber(v) or 200 end })

-- ================================================================
--  ESP TAB
-- ================================================================
EspTab:Toggle({ Title = "Enable ESP", Default = false,
...
EspTab:Toggle({
    Title = "Dropped Gun ESP", Description = "Highlight and label when a gun is on the map",
    Default = true,
    Callback = function(v)
...
EspTab:Divider()
EspTab:ColorPicker({ Title = "Murderer Color", Default = Color3.fromRGB(255,  40,  40), Callback = function(v) ESP_COLORS.Murderer = v end })
EspTab:ColorPicker({ Title = "Sheriff Color",  Default = Color3.fromRGB( 40, 130, 255), Callback = function(v) ESP_COLORS.Sheriff  = v end })
EspTab:ColorPicker({ Title = "Hero Color",     Default = Color3.fromRGB(255, 215,   0), Callback = function(v) ESP_COLORS.Hero     = v end })
EspTab:ColorPicker({ Title = "Innocent Color", Default = Color3.fromRGB(  0, 220,   0), Callback = function(v) ESP_COLORS.Innocent = v end })
